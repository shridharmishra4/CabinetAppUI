...
class Application:
    def __init__(self, output_path = "./"):
        ...
        ok, frame = self.vs.read() # get very first frame
    ...
    def draw_saved_states(self,frame_number):
        ...
        left_cell_state = self.global_cell_states[frame_number].get("left")
        right_cell_state = self.global_cell_states[frame_number].get("right")
        # print(frame_number,left_cell_state,right_cell_state)
        if left_cell_state or right_cell_state:
            for i in range(self.n*self.n):
                if left_cell_state:
                    left_coord = left_cell_state[i]["coordinates"]
                    if left_cell_state[i]["clicked"]:
                ...
                if right_cell_state:
                    right_coord = right_cell_state[i]["coordinates"]
                    if right_cell_state[i]["clicked"]:
        ...
        # if self.left_cell_state  is not None:
        #     for i in self.left_cell_state .keys():
        #         print(i,type(i))
    ...
    def next_frame(self,*_):
        ...
        if ok:  # frame captured without any errors
            ...
            if self.global_cell_states[self.current_frame]:
                self.draw_saved_states(self.current_frame)
                
            # print("hello",self.global_cell_states[1]["left"])
